CREATE OR REPLACE PROCEDURE sp_accessorialminmaxcriteria_merge_proc(
    OUT insertcount integer,
    OUT updatecount integer,
    OUT deletecount integer,
    OUT errornumber text,
    OUT errorstate text,
    OUT errorprocedure text,
    OUT errorline text,
    OUT errormessage text)
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
    CREATE TEMP TABLE merge_actions (
        table_name text,
        action     text,
        cny_cd text,
        asy_svc_typ_cd text,
        ccy_cd text,
        asy_trh_a decimal,
        asy_trh_typ_cd text,
        ccl_mth_typ_cd text,
        rec_eff_stt_dt date,
        rec_eff_end_dt date,
        svc_ra_cht_sts_cd text
    );

    WITH src_dedup AS (
        SELECT DISTINCT
               cny_cd,
               asy_svc_typ_cd,
               ccy_cd,
               asy_trh_a,
               asy_trh_typ_cd,
               ccl_mth_typ_cd,
               rec_eff_stt_dt,
               rec_eff_end_dt,
               svc_ra_cht_sts_cd,
               load_ref_te
          FROM tasytrh_stg
    ),
    cc_merge AS (
        MERGE INTO tasytrh AS tgt
        USING src_dedup AS src
        ON (
            tgt.cny_cd             = src.cny_cd AND
            tgt.asy_svc_typ_cd     = src.asy_svc_typ_cd AND
            tgt.ccy_cd             = src.ccy_cd AND
            tgt.asy_trh_typ_cd     = src.asy_trh_typ_cd AND
            tgt.rec_eff_stt_dt     = src.rec_eff_stt_dt AND
            tgt.svc_ra_cht_sts_cd  = src.svc_ra_cht_sts_cd
        )
        WHEN MATCHED AND (
            tgt.asy_trh_a      IS DISTINCT FROM src.asy_trh_a OR
            tgt.ccl_mth_typ_cd IS DISTINCT FROM src.ccl_mth_typ_cd OR
            tgt.rec_eff_end_dt IS DISTINCT FROM src.rec_eff_end_dt
        )
            THEN UPDATE SET
                asy_trh_a           = src.asy_trh_a,
                ccl_mth_typ_cd      = src.ccl_mth_typ_cd,
                rec_eff_end_dt      = src.rec_eff_end_dt,
                load_ref_te         = src.load_ref_te
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (
               cny_cd,
               asy_svc_typ_cd,
               ccy_cd,
               asy_trh_a,
               asy_trh_typ_cd,
               ccl_mth_typ_cd,
               rec_eff_stt_dt,
               rec_eff_end_dt,
               svc_ra_cht_sts_cd,
               load_ref_te
            ) VALUES (
               src.cny_cd,
               src.asy_svc_typ_cd,
               src.ccy_cd,
               src.asy_trh_a,
               src.asy_trh_typ_cd,
               src.ccl_mth_typ_cd,
               src.rec_eff_stt_dt,
               src.rec_eff_end_dt,
               src.svc_ra_cht_sts_cd,
               src.load_ref_te
            )
        WHEN NOT MATCHED BY SOURCE THEN
            DELETE
        RETURNING
            merge_action(), 
            COALESCE(tgt.cny_cd,            src.cny_cd)            AS cny_cd,
            COALESCE(tgt.asy_svc_typ_cd,    src.asy_svc_typ_cd)    AS asy_svc_typ_cd,
            COALESCE(tgt.ccy_cd,            src.ccy_cd)            AS ccy_cd,
            COALESCE(tgt.asy_trh_a,         src.asy_trh_a)         AS asy_trh_a,
            COALESCE(tgt.asy_trh_typ_cd,    src.asy_trh_typ_cd)    AS asy_trh_typ_cd,
            COALESCE(tgt.ccl_mth_typ_cd,    src.ccl_mth_typ_cd)    AS ccl_mth_typ_cd,
            COALESCE(tgt.rec_eff_stt_dt,    src.rec_eff_stt_dt)    AS rec_eff_stt_dt,
            COALESCE(tgt.rec_eff_end_dt,    src.rec_eff_end_dt)    AS rec_eff_end_dt,
            COALESCE(tgt.svc_ra_cht_sts_cd, src.svc_ra_cht_sts_cd) AS svc_ra_cht_sts_cd
    )
    INSERT INTO merge_actions (
        table_name, action,
        cny_cd, asy_svc_typ_cd, ccy_cd, asy_trh_a, asy_trh_typ_cd, ccl_mth_typ_cd, rec_eff_stt_dt, rec_eff_end_dt, svc_ra_cht_sts_cd
    )
    SELECT
        'tdstsvp', merge_action,
        cny_cd, asy_svc_typ_cd, ccy_cd, asy_trh_a, asy_trh_typ_cd, ccl_mth_typ_cd, rec_eff_stt_dt, rec_eff_end_dt, svc_ra_cht_sts_cd
    FROM cc_merge;

    SELECT
        COUNT(*) FILTER (WHERE action = 'INSERT'),
        COUNT(*) FILTER (WHERE action = 'UPDATE'),
        COUNT(*) FILTER (WHERE action = 'DELETE')
    INTO InsertCount, UpdateCount, DeleteCount
    FROM merge_actions;

    ErrorNumber    := NULL;
    ErrorState     := NULL;
    ErrorProcedure := 'sp_accessorialminmaxcriteria_merge_proc';
    ErrorLine      := NULL;
    ErrorMessage   := NULL;

EXCEPTION WHEN OTHERS THEN
    ErrorNumber    := SQLSTATE;
    ErrorState     := SQLSTATE;
    ErrorProcedure := 'sp_accessorialminmaxcriteria_merge_proc';
    GET STACKED DIAGNOSTICS ErrorLine = PG_EXCEPTION_CONTEXT;
    ErrorMessage   := SQLERRM;
    InsertCount := 0;
    UpdateCount := 0;
    DeleteCount := 0;
END;
$BODY$;
