CREATE OR REPLACE PROCEDURE sp_accessorialthreshold_merge_proc(
    OUT insertcount integer,
    OUT updatecount integer,
    OUT deletecount integer,
    OUT errornumber text,
    OUT errorstate text,
    OUT errorprocedure text,
    OUT errorline text,
    OUT errormessage text)
LANGUAGE 'plpgsql'
AS $BODY$
BEGIN
    CREATE TEMP TABLE merge_actions (
        table_name text,
        action     text,
        cny_cd text,
        asy_svc_typ_cd text,
        asy_dly_trh_qy integer,
        asy_wky_trh_qy integer,
        apv_sts_cd text,
        rec_eff_stt_dt date,
        rec_eff_end_dt date
    );

    WITH src_dedup AS (
        SELECT DISTINCT
               cny_cd,
               asy_svc_typ_cd,
               asy_dly_trh_qy,
               asy_wky_trh_qy,
               apv_sts_cd,
               rec_eff_stt_dt,
               rec_eff_end_dt,
               load_ref_te
          FROM tfputrh_stg
    ),
    cc_merge AS (
        MERGE INTO tfputrh AS tgt
        USING src_dedup AS src
        ON (
            tgt.cny_cd          = src.cny_cd AND
            tgt.asy_svc_typ_cd  = src.asy_svc_typ_cd AND
            tgt.apv_sts_cd      = src.apv_sts_cd AND
            tgt.rec_eff_stt_dt  = src.rec_eff_stt_dt
        )
        WHEN MATCHED AND (
            tgt.asy_dly_trh_qy IS DISTINCT FROM src.asy_dly_trh_qy OR
            tgt.asy_wky_trh_qy IS DISTINCT FROM src.asy_wky_trh_qy OR
            tgt.rec_eff_end_dt IS DISTINCT FROM src.rec_eff_end_dt
        ) THEN UPDATE SET
            asy_dly_trh_qy = src.asy_dly_trh_qy,
            asy_wky_trh_qy = src.asy_wky_trh_qy,
            rec_eff_end_dt = src.rec_eff_end_dt,
            load_ref_te    = src.load_ref_te
        WHEN NOT MATCHED BY TARGET THEN
            INSERT (
                cny_cd,
                asy_svc_typ_cd,
                asy_dly_trh_qy,
                asy_wky_trh_qy,
                apv_sts_cd,
                rec_eff_stt_dt,
                rec_eff_end_dt,
                load_ref_te
            ) VALUES (
                src.cny_cd,
                src.asy_svc_typ_cd,
                src.asy_dly_trh_qy,
                src.asy_wky_trh_qy,
                src.apv_sts_cd,
                src.rec_eff_stt_dt,
                src.rec_eff_end_dt,
                src.load_ref_te
            )
        WHEN NOT MATCHED BY SOURCE THEN
            DELETE
        RETURNING
            merge_action(),
            COALESCE(tgt.cny_cd,         src.cny_cd)        AS cny_cd,
            COALESCE(tgt.asy_svc_typ_cd, src.asy_svc_typ_cd) AS asy_svc_typ_cd,
            COALESCE(tgt.asy_dly_trh_qy, src.asy_dly_trh_qy) AS asy_dly_trh_qy,
            COALESCE(tgt.asy_wky_trh_qy, src.asy_wky_trh_qy) AS asy_wky_trh_qy,
            COALESCE(tgt.apv_sts_cd,     src.apv_sts_cd)     AS apv_sts_cd,
            COALESCE(tgt.rec_eff_stt_dt, src.rec_eff_stt_dt) AS rec_eff_stt_dt,
            COALESCE(tgt.rec_eff_end_dt, src.rec_eff_end_dt) AS rec_eff_end_dt
    )
    INSERT INTO merge_actions(
        table_name, action,
        cny_cd, asy_svc_typ_cd, asy_dly_trh_qy, asy_wky_trh_qy, apv_sts_cd, rec_eff_stt_dt, rec_eff_end_dt
    )
    SELECT 'tfputrh', merge_action,
           cny_cd, asy_svc_typ_cd, asy_dly_trh_qy, asy_wky_trh_qy, apv_sts_cd, rec_eff_stt_dt, rec_eff_end_dt
    FROM cc_merge;

    SELECT
        COUNT(*) FILTER (WHERE action = 'INSERT'),
        COUNT(*) FILTER (WHERE action = 'UPDATE'),
        COUNT(*) FILTER (WHERE action = 'DELETE')
    INTO InsertCount, UpdateCount, DeleteCount
    FROM merge_actions;

    ErrorNumber    := NULL;
    ErrorState     := NULL;
    ErrorProcedure := 'sp_accessorialthreshold_merge_proc';
    ErrorLine      := NULL;
    ErrorMessage   := NULL;

EXCEPTION WHEN OTHERS THEN
    ErrorNumber    := SQLSTATE;
    ErrorState     := SQLSTATE;
    ErrorProcedure := 'sp_accessorialthreshold_merge_proc';
    GET STACKED DIAGNOSTICS ErrorLine = PG_EXCEPTION_CONTEXT;
    ErrorMessage   := SQLERRM;
    InsertCount := 0;
    UpdateCount := 0;
    DeleteCount := 0;
END;
$BODY$;