CREATE OR REPLACE PROCEDURE sp_informationalaccessorialthreshold_merge_proc(
...
BEGIN
    ...
    cc_merge AS (
        ...
        WHEN MATCHED AND (
            ...
            tgt.dtr_cri_vlu_qy IS DISTINCT FROM src.dtr_cri_vlu_qy
        )
            THEN UPDATE SET
    ...
    INSERT INTO merge_actions (
        table_name, action,
        cny_cd, asy_svc_typ_cd, tm_prd_typ_cd, dtr_cri_eff_dt, cus_csf_typ_cd
    )
    SELECT
        'tinftrh', merge_action,
        cny_cd, asy_svc_typ_cd, tm_prd_typ_cd, dtr_cri_eff_dt, cus_csf_typ_cd
    FROM cc_merge;
    ...
    ErrorNumber    := NULL;
    ErrorState     := NULL;
    ErrorProcedure := 'sp_informationalaccessorialthreshold_merge_proc';
    ErrorLine      := NULL;
    ErrorMessage   := NULL;
