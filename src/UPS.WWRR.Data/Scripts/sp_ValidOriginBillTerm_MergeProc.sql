CREATE OR REPLACE PROCEDURE sp_validoriginbillterm_merge_proc(
...
BEGIN
    ...
    INSERT INTO merge_actions (
        table_name, action,
        gpn_xpt_cny_cd, bil_ter_typ_cd, apv_sts_cd, rec_eff_stt_dt, rec_eff_end_dt
    )
    SELECT 'tvorgbt', merge_action,
           gpn_xpt_cny_cd, bil_ter_typ_cd, apv_sts_cd, rec_eff_stt_dt, rec_eff_end_dt
    FROM cc_merge;
    ...
    ErrorNumber    := NULL;
    ErrorState     := NULL;
    ErrorProcedure := 'sp_validoriginbillterm_merge_proc';
    ErrorLine      := NULL;
    ErrorMessage   := NULL;
