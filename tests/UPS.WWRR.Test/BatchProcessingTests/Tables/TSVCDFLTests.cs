...
public class TSVCDFLTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TSVCDFL_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TSVCDFL_6401.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TSVCDFL_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TSVCDFL_6402.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
        _repo.Verify(r => r.UpdateStatusAsync(6402, LoadStatus.FailedValidation, null, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task CopyBatchLoadAsync_TSVCDFL_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TSVCDFL_6411.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
    ...
    public async Task PerformMergeLoadAsync_TSVCDFL_FailedMerge_UpdatesFailed()
    {
        ...
        _repo.Verify(r => r.UpdateStatusAsync(6430, LoadStatus.Failed, null, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task PerformMergeLoadAsync_TSVCDFL_Success_Processed()
    {
        ...
        _repo.Setup(r => r.UpdateLoadReferenceAsync(It.IsAny<string>(), It.IsAny<string>(), 6431, It.IsAny<long>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(1);
        var sut = CreateSut();
        await InvokeAsync<object>(sut, "PerformMergeLoadAsync", new List<DataLoad> { load }, CancellationToken.None);
