...
public class TLMTVLUTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TLMTVLU_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.FileExistsAsync("TLMTVLU_4401.csv", It.IsAny<CancellationToken>())).ReturnsAsync(true);
        _storage.Setup(s => s.DownloadFile("TLMTVLU_4401.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        _storage.Setup(s => s.VerifyFileSizeAsync("TLMTVLU_4401.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(true);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TLMTVLU_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.FileExistsAsync("TLMTVLU_4402.csv", It.IsAny<CancellationToken>())).ReturnsAsync(true);
        _storage.Setup(s => s.DownloadFile("TLMTVLU_4402.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        _storage.Setup(s => s.VerifyFileSizeAsync("TLMTVLU_4402.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).ReturnsAsync(true);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
        _repo.Verify(r => r.UpdateStatusAsync(4402, LoadStatus.FailedValidation, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task CopyBatchLoadAsync_TLMTVLU_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TLMTVLU_4411.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    public async Task PerformMergeLoadAsync_TLMTVLU_FailedMerge_UpdatesFailed()
    {
        ...
        _repo.Verify(r => r.UpdateStatusAsync(4430, LoadStatus.Failed, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
