...
public class TSIARAVTests : BatchProcessorTests
{
    [Fact]
    public async Task ValidateLoadsAsync_TSIARAV_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TSIARAV_501.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    [Fact]
    public async Task ValidateLoadsAsync_TSIARAV_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TSIARAV_502.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
    ...
    public async Task CopyBatchLoadAsync_TSIARAV_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TSIARAV_511.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
    ...
    [Fact]
    public async Task PerformMergeLoadAsync_TSIARAV_FailedMerge_UpdatesFailed()
    {
        var load = new DataLoad { Id = 530, LoadTableName = "tsiarav", FileLocation = "gs://bucket/TSIARAV_530.csv" };
    ...
    [Fact]
    public async Task PerformMergeLoadAsync_TSIARAV_Success_Processed()
    {
        var load = new DataLoad { Id = 531, LoadTableName = "tsiarav", FileLocation = "gs://bucket/TSIARAV_531.csv" };
        _repo.Setup(r => r.ExecuteMergeStoredProcedureAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
