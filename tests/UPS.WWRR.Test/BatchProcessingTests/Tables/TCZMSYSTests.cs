...
public class TCZMSYSTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TCZMSYS_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TCZMSYS_301.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TCZMSYS_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TCZMSYS_302.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
    ...
    public async Task CopyBatchLoadAsync_TCZMSYS_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TCZMSYS_311.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
