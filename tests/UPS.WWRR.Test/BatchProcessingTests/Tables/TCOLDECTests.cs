...
public class TCOLDECTests : BatchProcessorTests
{
    ...
    {
        _storage.Setup(s => s.DiscoverReceiptLogFileAsync(Bucket, It.IsAny<CancellationToken>())).ReturnsAsync("receipt.csv");
        var content = "FileExtractName,Source\nTCOLDEC_12345.csv,SRC";
        _storage.Setup(s => s.GetFileAsString(Bucket, "receipt.csv")).ReturnsAsync(content);
        _repo.Setup(r => r.ExistsAsync("TCOLDEC", 12345, It.IsAny<CancellationToken>())).ReturnsAsync(false);
        ...
        var list = await InvokeAsync<List<DataLoad>>(sut, "BuildLoadsAsync", CancellationToken.None);
        Assert.Single(list);
        Assert.Equal("TCOLDEC", list[0].LoadTableName);
    ...
    {
        var load = new DataLoad { Id = 10, LoadTableName = "tcoldec", FileLocation = "gs://bucket/TCOLDEC_10.csv" };
        _validator.Setup(v => v.ValidateCsvAsync<UPS.WWRR.Business.DTO.Models.LoadTableDto.ColumnDecodeDto>(It.IsAny<string>()))
            .ReturnsAsync(new CsvValidationResponse(true, new List<string>()));
        _storage.Setup(s => s.DownloadFile(Bucket, "TCOLDEC_10.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    {
        var load = new DataLoad { Id = 11, LoadTableName = "tcoldec", FileLocation = "gs://bucket/TCOLDEC_11.csv" };
        _validator.Setup(v => v.ValidateCsvAsync<UPS.WWRR.Business.DTO.Models.LoadTableDto.ColumnDecodeDto>(It.IsAny<string>()))
            .ReturnsAsync(new CsvValidationResponse(false, new List<string> { "e" }));
        _storage.Setup(s => s.DownloadFile(Bucket, "TCOLDEC_11.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
    ...
    {
        var load = new DataLoad { Id = 21, LoadTableName = "tcoldec", FileLocation = "gs://bucket/TCOLDEC_21.csv" };
        _storage.Setup(s => s.DownloadFile(Bucket, "TCOLDEC_21.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        _copy.Setup(c => c.CopyAsync(It.IsAny<string>(), It.IsAny<TableConfigurationRequest>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new CopyBatchResultDto { TableName = "tcoldec_stg", SourceFile = "temp", RowsLoaded = 5, TotalRowsAttempted = 5, StartedAt = DateTimeOffset.UtcNow, CompletedAt = DateTimeOffset.UtcNow });
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
    ...
    {
        var load = new DataLoad { Id = 30, LoadTableName = "tcoldec", FileLocation = "gs://bucket/TCOLDEC_30.csv" };
    ...
    {
        var load = new DataLoad { Id = 31, LoadTableName = "tcoldec", FileLocation = "gs://bucket/TCOLDEC_31.csv" };
        _repo.Setup(r => r.ExecuteMergeStoredProcedureAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new MergeResult(1, 2, 0, null, null, "sp", null, null));
        _repo.Setup(r => r.UpdateLoadReferenceAsync(It.IsAny<string>(), It.IsAny<string>(), 31, It.IsAny<long>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(5);
        var sut = CreateSut();
        await InvokeAsync<object>(sut, "PerformMergeLoadAsync", new List<DataLoad> { load }, CancellationToken.None);
