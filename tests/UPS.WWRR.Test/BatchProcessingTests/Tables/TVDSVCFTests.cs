...
public class TVDSVCFTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TVDSVCF_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TVDSVCF_3601.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TVDSVCF_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TVDSVCF_3602.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
    ...
    public async Task CopyBatchLoadAsync_TVDSVCF_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TVDSVCF_3611.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
