...
public class TSVCACPTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TSVCACP_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TSVCACP_5401.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TSVCACP_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TSVCACP_5402.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
        _repo.Verify(r => r.UpdateStatusAsync(5402, LoadStatus.FailedValidation, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task CopyBatchLoadAsync_TSVCACP_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TSVCACP_5411.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    public async Task PerformMergeLoadAsync_TSVCACP_FailedMerge_UpdatesFailed()
    {
        ...
        _repo.Verify(r => r.UpdateStatusAsync(5430, LoadStatus.Failed, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
