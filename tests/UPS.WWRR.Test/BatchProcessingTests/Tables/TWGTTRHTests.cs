...
public class TWGTTRHTests : BatchProcessorTests
{
    ...
    public async Task BuildLoadsAsync_ValidSingleLoad_Inserts()
    {
        ...
        var list = await InvokeAsync<List<DataLoad>>(sut, "BuildLoadsAsync", CancellationToken.None);
        Assert.Single(list);
        Assert.Equal("TWGTTRH", list[0].LoadTableName);
    ...
    public async Task ValidateLoadsAsync_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TWGTTRH_10.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TWGTTRH_11.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
        _repo.Verify(r => r.UpdateStatusAsync(11, LoadStatus.FailedValidation, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task CopyBatchLoadAsync_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TWGTTRH_21.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    {
        var load = new DataLoad { Id = 30, LoadTableName = "twgttrh", FileLocation = "gs://bucket/TWGTTRH_30.csv" };
        ...
        _repo.Verify(r => r.UpdateStatusAsync(30, LoadStatus.Failed, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task PerformMergeLoadAsync_Success_Processed()
    {
        var load = new DataLoad { Id = 31, LoadTableName = "twgttrh", FileLocation = "gs://bucket/TWGTTRH_31.csv" };
        _repo.Setup(r => r.ExecuteMergeStoredProcedureAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
