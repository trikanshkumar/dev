...
public class TCNYASYTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TCNYASY_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TCNYASY_401.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TCNYASY_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TCNYASY_402.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
    ...
    public async Task CopyBatchLoadAsync_TCNYASY_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TCNYASY_411.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
