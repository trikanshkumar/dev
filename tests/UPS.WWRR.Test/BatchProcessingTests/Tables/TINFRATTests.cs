...
public class TINFRATTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TINFRAT_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TINFRAT_1371.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TINFRAT_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TINFRAT_1372.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
        _repo.Verify(r => r.UpdateStatusAsync(1372, LoadStatus.FailedValidation, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task CopyBatchLoadAsync_TINFRAT_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile("TINFRAT_1381.csv", It.IsAny<string>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
    ...
    public async Task PerformMergeLoadAsync_TINFRAT_FailedMerge_UpdatesFailed()
    {
        ...
        _repo.Verify(r => r.UpdateStatusAsync(1390, LoadStatus.Failed, It.IsAny<DateTime>(), It.IsAny<CancellationToken>()), Times.Once);
    }
