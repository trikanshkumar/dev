...
public class TINFCHGTests : BatchProcessorTests
{
    ...
    public async Task ValidateLoadsAsync_TINFCHG_SetsReadyToProcess_OnSuccess()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TINFCHG_1101.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
    ...
    public async Task ValidateLoadsAsync_TINFCHG_SetsFailedValidation_OnFailure()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TINFCHG_1102.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
        var sut = CreateSut();
        var tempFiles = new Dictionary<string, string>();
        await InvokeAsync<object>(sut, "ValidateLoadsAsync", new List<DataLoad> { load }, CancellationToken.None, tempFiles);
        _repo.Verify(r => r.UpdateStatusAsync(1102, LoadStatus.FailedValidation, null, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task CopyBatchLoadAsync_TINFCHG_SetsProcessingOnStart()
    {
        ...
        _storage.Setup(s => s.DownloadFile(Bucket, "TINFCHG_1111.csv", It.IsAny<string>())).Returns(Task.CompletedTask);
    ...
    public async Task PerformMergeLoadAsync_TINFCHG_FailedMerge_UpdatesFailed()
    {
        ...
        _repo.Verify(r => r.UpdateStatusAsync(1130, LoadStatus.Failed, null, It.IsAny<CancellationToken>()), Times.Once);
    }
